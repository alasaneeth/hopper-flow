    ...
    <div>
      ...
      <div className={`max-w-2xl mx-auto rounded-2xl border p-8
        ...
        <div className={`text-center mb-8 pb-6 border-b ${isDark ? 'border-[#232323]' : 'border-gray-100'}`}>
          ...
          <p className="text-sm text-gray-500 mt-1">String Hopper Business Management</p>
        </div>
        ...
        <div className={`rounded-lg px-4 py-3 mb-6 ${isDark ? 'bg-[#0f0f0f]' : 'bg-gray-50'}`}>
        ...
        <table className="w-full text-sm mb-6">
          <thead>
            <tr className={`border-b ${isDark ? 'border-[#232323]' : 'border-gray-200'}`}>
              <th className="text-left py-3 text-gray-500 text-xs uppercase tracking-wider">Item</th>
              <th className="text-right py-3 text-gray-500 text-xs uppercase tracking-wider">Qty</th>
              <th className="text-right py-3 text-gray-500 text-xs uppercase tracking-wider">Unit Price</th>
              <th className="text-right py-3 text-gray-500 text-xs uppercase tracking-wider">Amount</th>
            </tr>
          </thead>
          <tbody>
            {order.whiteHopperCount > 0 && (
              <tr className={`border-b ${isDark ? 'border-[#1a1a1a]' : 'border-gray-50'}`}>
                <td className={`py-3 ${isDark ? 'text-white' : 'text-gray-900'}`}>White String Hopper</td>
                <td className="text-right py-3 text-gray-400">{order.whiteHopperCount}</td>
                <td className="text-right py-3 text-gray-400">Rs. {order.whiteHopperPrice.toFixed(2)}</td>
                <td className={`text-right py-3 font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
            ...
            {order.redHopperCount > 0 && (
              <tr className={`border-b ${isDark ? 'border-[#1a1a1a]' : 'border-gray-50'}`}>
                <td className={`py-3 ${isDark ? 'text-white' : 'text-gray-900'}`}>Red String Hopper</td>
                <td className="text-right py-3 text-gray-400">{order.redHopperCount}</td>
                <td className="text-right py-3 text-gray-400">Rs. {order.redHopperPrice.toFixed(2)}</td>
                <td className={`text-right py-3 font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className={`space-y-2 pt-4 border-t ${isDark ? 'border-[#232323]' : 'border-gray-200'}`}>
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="text-green-500">Rs. {order.paidAmount.toLocaleString()}</span>
          </div>
          <div className={`flex justify-between text-lg font-bold pt-3 border-t ${isDark ? 'border-[#232323]' : 'border-gray-200'}`}>
            <span className={isDark ? 'text-white' : 'text-gray-900'}>Balance Due</span>
        ...
        {order.payments?.length > 0 && (
          <div className={`mt-6 pt-6 border-t ${isDark ? 'border-[#232323]' : 'border-gray-200'}`}>
            <p className="text-xs text-gray-500 mb-3 uppercase tracking-wider">Payment History</p>
      ...
      <Modal show={showPaymentModal} onClose={() => setShowPaymentModal(false)} isDark={isDark} maxWidth="max-w-md">
