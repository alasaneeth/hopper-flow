...
const PreparationPage = () => {
  ...
  const availableRiceStock = stocks.find(s => s.riceType === parseInt(form.productType))
  ...
  return (
    <div>
      <PageHeader
        title="Preparation"
        subtitle="Team 1 — Rice to Dough preparation"
        isDark={isDark}
        action={
          <button
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <StatCard label="Total Preparations" value={totalPreparations} isDark={isDark} />
        <StatCard label="Total Rice Used" value={totalRiceUsed.toFixed(1)} suffix="kg" color="text-red-400" isDark={isDark} />
        <StatCard label="Total Dough Produced" value={totalDoughProduced.toFixed(1)} suffix="kg" color="text-green-500" isDark={isDark} />
      </div>

      <div className={`rounded-xl p-5 border mb-6 grid grid-cols-2 gap-6
        ${isDark ? 'bg-[#141414] border-[#232323]' : 'bg-white border-gray-200'}`}>
        <div>
          <p className="text-xs text-gray-500 mb-3 uppercase tracking-wider">Rice Stock</p>
          <div className="space-y-2">
            {stocks.map(s => (
              <div key={s.id} className="flex justify-between items-center">
                <span className="text-sm text-gray-500">{s.riceTypeName}</span>
                <span className={`text-sm font-semibold ${s.isLowStock ? 'text-red-400' : 'text-green-500'}`}>
        ...
        <div>
          <p className="text-xs text-gray-500 mb-3 uppercase tracking-wider">Dough Stock</p>
          <div className="space-y-2">
        ...
        <table className="w-full">
          <thead>
            <tr className={`border-b ${isDark ? 'border-[#1e1e1e]' : 'border-gray-100'}`}>
              {['#', 'Date', 'Type', 'Rice Used', 'Milling', 'Sieving', 'Dough Produced', 'Notes', ''].map(h => (
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="9" className="text-center py-12 text-gray-600">Loading...</td></tr>
            ) : preparations.length === 0 ? (
              <tr><td colSpan="9" className="text-center py-12 text-gray-600">No preparations yet — start one!</td></tr>
            ) : (
              preparations.map((p, i) => (
                <tr key={p.id}
                  ...
                  <td className="px-6 py-4">
                    <span className={`text-xs font-medium ${p.millingDone ? 'text-green-500' : 'text-gray-600'}`}>
                      {p.millingDone ? '✓ Done' : '✗ No'}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`text-xs font-medium ${p.sievingDone ? 'text-green-500' : 'text-gray-600'}`}>
      ...
      <Modal show={showModal} onClose={resetForm} isDark={isDark}>
        <div className={`px-6 py-5 border-b ${isDark ? 'border-[#232323]' : 'border-gray-100'}`}>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Team 1 — Rice to Dough</p>
        </div>
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-3">
          ...
          {availableRiceStock && (
            <div className={`rounded-lg px-4 py-2.5 border
              ...
              <p className={`text-sm font-semibold mt-0.5 ${availableRiceStock.isLowStock ? 'text-red-400' : 'text-green-500'}`}>
                {availableRiceStock.quantityKg} kg{availableRiceStock.isLowStock && ' ⚠️ Low!'}
              </p>
            </div>
          )}
          ...
          <div className={`grid grid-cols-2 gap-3 p-4 rounded-lg border
            ...
            <div className="flex items-center gap-3">
              <input
                type="checkbox" id="milling"
                checked={form.millingDone}
                onChange={e => setForm({ ...form, millingDone: e.target.checked })}
                className="w-4 h-4 accent-green-500"
              />
              <label htmlFor="milling" className={`text-sm ${isDark ? 'text-white' : 'text-gray-900'}`}>
                Milling Done
              </label>
            </div>
            <div className="flex items-center gap-3">
              <input
                type="checkbox" id="sieving"
                checked={form.sievingDone}
                onChange={e => setForm({ ...form, sievingDone: e.target.checked })}
                className="w-4 h-4 accent-green-500"
              />
              <label htmlFor="sieving" className={`text-sm ${isDark ? 'text-white' : 'text-gray-900'}`}>
                Sieving Done
              </label>
            </div>
          </div>
